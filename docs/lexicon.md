...
| token | Anything Scryfall ships that can never be a deck candidate: tokens, double-faced tokens, emblems, art series, planar, scheme and vanguard cards, and anything printed in a set of type `token`, such as Role tokens and dungeons. The Scryfall layout or the token set type decides (ADR-0008). Kept in the catalog's token section, excluded from the collection view; owned when any lot's printing shares its oracle_id. Wider than the game's own meaning. | #3 |
