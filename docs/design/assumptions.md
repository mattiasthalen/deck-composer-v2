...
| A17 | Cards placed in ManaBox decks appear in the collection export with Binder Type `deck`. | Ingest, table store, composer | Unverified. No ManaBox decks exist today | A re-export after creating a ManaBox deck lacks those rows or carries another type. Until verified, scoping by ManaBox deck has no data. |
| A18 | The Added timestamp is stable per row across exports. | Ingest | Unverified. One export seen | A re-export whose diff changes Added on otherwise unchanged lots. Then drop the column; the lot key already excludes it. |
